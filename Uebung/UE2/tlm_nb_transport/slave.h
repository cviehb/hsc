...
static ofstream sout("slave.txt");
...
struct Slave : sc_module {
  ...
  SC_CTOR(Slave)
      : socket("bus_rw"), n_trans(0) // number of transactions in progress
        ,
  ...
  void peq_cb(tlm::tlm_generic_payload &trans, const tlm::tlm_phase &phase) {
    ...
    case tlm::BEGIN_REQ:
#ifdef DEBUG
      cout << hex << trans.get_address() << " BEGIN_REQ at " << sc_time_stamp()
           << endl;
#endif
      ...
      // put back-pressure on intitiator by deferring END_REQ until pipeline is
      // clear allow only 2 pending transactions
      if (n_trans == 2) {
