...
// Version  2, 16 July 2008  Only generate ref_count > 1 warning from 1st
// checker of path Version  3, 17 July 2008  Support compilation under
// SystemC 2.1.v1 Version  4, 12 Aug  2008  Add header #include <map> Version 5,
// 08 Sep  2008  Fix bugs in message text Version  6, 01 Aug  2010  Update
// messages to refer to OSCI TLM-2.0 LRM of July 2009 Version  7, 25 Oct  2011
// Minor bug fix for certain compilers: replace u_char with uchar_t Version  8,
// 02 Nov  2011  Support the endianness conversion functions by excluding the
//                           tlm_endian_context extension from the protocol
//                           checks
// Version  9, 17 Aug  2012  Fix LRM reference on line 805 (should be 8.2.11 a)
// [NOT YET RELEASED] Version 10,  3 Jan  2013  Updated messages to refer to
// IEEE Std 1666-2011, the combined SystemC + TLM-2.0 LRM
//                           Added checks related to the generic payload option
//                           attribute

// TLM-2.0 Base Protocol Compliance Checker

/*
Instantiate this checker module in-line between initiator and target, initiator
and interconnect, or interconnect and target by binding the target_socket and
initiator_socket Binding two checkers either side of an interconnect component,
or interleaving a series of checkers with interconnect components, will enable
some deeper checks as against having just a single checker
...
The checks are relatively expensive, hence by default the number of checks is
limited. A maximum number can be set explicitly by calling set_num_checks(max)
Checking can be deactivated at any time by calling set_num_checks(0)
All checkers decrement a single global count, because having some checkers
running and others not can cause bogus violation reports It is not permitted to
turn checks on by calling set_num_checks() once checking has been deactivated,
because this could cause bogus violation reports

The DMI and debug checks are unaffected by the num_checks count (because they
are cheap)

The error messages contain User Manual references

The checker is designed to be used with a transaction pool: otherwise it could
consume a lot of memory. The checker keeps a local copy of each transaction
object Failures are reported with a severity of SC_ERROR. The actions may be
overridden by calling: sc_report_handler::set_actions("tlm2_protocol_checker",
...);
...
Must not have decreasing timing annotations on calls to or returns from
nb_transport_fw/bw Phase extensions permitted and ignored Must not call
b_transport during nb_transport phase sequence and vice-versa
...
An interconnect component that sets extensions with no memory manager should
also clear them (Does not bother with these memory manager checks for DMI and
debug)

Transaction object must be properly initialized
Many generic payload attributes must not be modified during the transaction
lifetime Transaction object must not be re-allocated for a new transaction while
still in use DMI descriptor must be properly initialized Debug transaction must
be properly initialized Debug byte count must be less than data_length

Checks that require multiple checkers to be instantiated along a transaction
path: The BEGIN_RESP path must be the reverse of the BEGIN_REQ path Transaction
object must not be sent with BEGIN_REQ while participating in a previous
response Interconnect component must not set response status attribute to
TLM_OK_RESPONSE Interconnect component must not modify data array on the
response path
...
When gp_option is used, other gp attributes must be initalized and used as per
...
// Global variable used for checks involving multiple checkers along a
// transaction path
...
public:
  // Instantiate and bind checker inline between an existing pair of initiator
  // and target sockets
  ...
  static void set_num_checks(sc_dt::uint64 n) {
    if (num_checks == 0)
      SC_REPORT_FATAL("tlm2_protocol_checker",
                      "Method set_num_checks called after checking has stopped "
                      "due to maximum number of checks being reached");
    num_checks = n;
  }

  static sc_dt::uint64 get_num_checks() { return num_checks; }

  // TLM-2.0 interface methods for initiator and target sockets, instrumented
  // with checks
...
private:
  ...
  void nb_transport_fw_pre_checks(tlm::tlm_generic_payload &trans,
                                  tlm::tlm_phase &phase,
                                  sc_core::sc_time &delay);

  void nb_transport_fw_post_checks(tlm::tlm_generic_payload &trans,
                                   tlm::tlm_phase &start_phase,
                                   tlm::tlm_phase &phase,
                                   sc_core::sc_time &delay,
                                   tlm::tlm_sync_enum status);

  void nb_transport_bw_pre_checks(tlm::tlm_generic_payload &trans,
                                  tlm::tlm_phase &phase,
                                  sc_core::sc_time &delay);

  void nb_transport_bw_post_checks(tlm::tlm_generic_payload &trans,
                                   tlm::tlm_phase &phase,
                                   sc_core::sc_time &delay,
                                   tlm::tlm_sync_enum status);

  void nb_transport_response_checks(tlm::tlm_generic_payload &trans,
                                    tlm::tlm_phase &phase,
                                    sc_core::sc_time &delay, const char *txt2,
...
#define BOILERPLATE                                                            \
  template <unsigned int BUSWIDTH> void tlm2_base_protocol_checker<BUSWIDTH>::

BOILERPLATE
b_transport_pre_checks(tlm::tlm_generic_payload &trans,
                       sc_core::sc_time &delay) {
  ++m_map[&trans].b_call;

  if (trans.has_mm() && trans.get_ref_count() == 0) {
    txt << "Transaction passed to b_transport with memory manager and "
...
BOILERPLATE
b_transport_post_checks(tlm::tlm_generic_payload &trans,
                        sc_core::sc_time &delay) {
...
BOILERPLATE
nb_transport_fw_pre_checks(tlm::tlm_generic_payload &trans,
  ...
  case tlm::BEGIN_REQ:
    ...
    {
      txt << "Phase " << phase
          << " sent out-of-sequence on forward path, detected in "
             "nb_transport_fw";
      tlm2error(trans, "15.2.4");
    }

    if (m_request_in_progress) {
      txt << "Transaction violates BEGIN_REQ exclusion rule, detected in "
             "nb_transport_fw";
  ...
  if (sc_core::sc_time_stamp() + delay < m_map[&trans].time) {
    txt << "nb_transport_fw called with decreasing timing annotation:"
        << " delay = " << delay
        << ", sc_time_stamp() + delay from previous call = "
        << m_map[&trans].time;
...
BOILERPLATE
nb_transport_fw_post_checks(tlm::tlm_generic_payload &trans,
                            tlm::tlm_phase &start_phase, tlm::tlm_phase &phase,
                            sc_core::sc_time &delay,
                            tlm::tlm_sync_enum status) {
  if (status == tlm::TLM_UPDATED) {
    nb_transport_response_checks(trans, phase, delay, "(forward) return",
                                 "Return from nb_transport_fw",
                                 "nb_transport_fw");
  ...
  // Transaction object should not be re-allocated, even during the END_RESP
  // phase
...
BOILERPLATE
nb_transport_bw_pre_checks(tlm::tlm_generic_payload &trans,
                           tlm::tlm_phase &phase, sc_core::sc_time &delay) {
  if (!trans.has_mm()) {
  ...
  nb_transport_response_checks(trans, phase, delay, "backward",
                               "nb_transport_bw called", "nb_transport_bw");
}

BOILERPLATE
nb_transport_bw_post_checks(tlm::tlm_generic_payload &trans,
                            tlm::tlm_phase &phase, sc_core::sc_time &delay,
                            tlm::tlm_sync_enum status) {
  if (status == tlm::TLM_UPDATED) {
    switch (phase) {
    case tlm::BEGIN_REQ:
      txt << "Phase " << phase
          << " sent out-of-sequence on (backward) return path, detected in "
             "nb_transport_bw";
      tlm2error(trans, "15.2.4");
      break;
...
BOILERPLATE
nb_transport_response_checks(tlm::tlm_generic_payload &trans,
                             tlm::tlm_phase &phase, sc_core::sc_time &delay,
  ...
  if (trans.is_response_ok())
    if (shared_map[&trans].response_in_progress &&
      ...
      txt << "Interconnect component sets response status attribute to "
             "TLM_OK_RESPONSE"
          << ", detected in " << txt4;
...
BOILERPLATE
check_initial_state(tlm::tlm_generic_payload &trans, const char *txt2) {
  if (num_checks > 0) {
    --num_checks;
    if (num_checks == 0)
      SC_REPORT_INFO(
          "tlm2_protocol_checker",
  ...
  if (trans.has_mm() && trans.get_ref_count() > 1 &&
    ...
    txt << "New transaction passed to " << txt2
        << " with reference count = " << trans.get_ref_count();
  ...
  if (trans.get_data_length() == 0 &&
      trans.get_command() != tlm::TLM_IGNORE_COMMAND) {
    txt << "Transaction not properly initialized: data_langth == 0, detected "
           "in "
        << txt2;
  ...
  if (trans.get_byte_enable_ptr() != 0 && trans.get_byte_enable_length() == 0) {
    txt << "Transaction not properly initialized: "
        << "byte_enable_ptr != 0 and byte_enable_length == 0, detected in "
        << txt2;
    tlm2error(trans, "14.14 f)");
  }
  if (trans.get_streaming_width() == 0) {
    txt << "Transaction not properly initialized: streaming_width == 0, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.15 f)");
  }
  if (trans.is_dmi_allowed()) {
    txt << "Transaction not properly initialized: dmi_allowed == true, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.16");
  }
  if (trans.get_response_status() != tlm::TLM_INCOMPLETE_RESPONSE) {
    txt << "Transaction not properly initialized: response_status != "
           "TLM_INCOMPLETE_RESPONSE, detected in "
        << txt2;
    tlm2error(trans, "14.17 e)");
  }
  if (trans.get_gp_option() != tlm::TLM_MIN_PAYLOAD) {
    txt << "Transaction not properly initialized: gp_option != "
           "TLM_MIN_PAYLOAD, detected in "
        << txt2;
  ...
  if (gp == 0)
    gp = new tlm::tlm_generic_payload; // Memory leak: transactions are never
                                       // cleared from map
  else {
  ...
  if (shared_map[&trans].response_in_progress) {
    txt << "Transaction object sent with BEGIN_REQ while still being used on a "
           "previous response path, detected in "
        << txt2;
...
BOILERPLATE
remember_gp_option(tlm::tlm_generic_payload &trans) {
...
BOILERPLATE
check_trans_not_modified(tlm::tlm_generic_payload &trans, const char *txt2) {
  tlm::tlm_generic_payload *init = m_map[&trans].gp;

  if (trans.get_command() != init->get_command()) {
    txt << "Command attribute modified during transaction lifetime, detected "
           "in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (trans.get_data_ptr() != m_map[&trans].data_ptr) {
    txt << "Data pointer attribute modified during transaction lifetime, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (trans.get_data_length() != init->get_data_length()) {
    txt << "Data length attribute modified during transaction lifetime, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (trans.get_command() == tlm::TLM_WRITE_COMMAND)
    for (unsigned int i = 0; i < init->get_data_length(); i++)
      if (trans.get_data_ptr()[i] != init->get_data_ptr()[i]) {
        txt << "Data array modified during transaction lifetime, detected in "
            << txt2;
        tlm2error(trans, "14.7");
      }
  if (trans.get_byte_enable_ptr() != m_map[&trans].byte_enable_ptr) {
    txt << "Byte enable pointer attribute modified during transaction "
           "lifetime, detected in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (trans.get_byte_enable_length() != init->get_byte_enable_length()) {
    txt << "Byte enable length attribute modified during transaction lifetime, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (trans.get_byte_enable_ptr())
    for (unsigned int i = 0; i < init->get_byte_enable_length(); i++)
      if (trans.get_byte_enable_ptr()[i] != init->get_byte_enable_ptr()[i]) {
        txt << "Byte enable array modified during transaction lifetime, "
               "detected in "
            << txt2;
        tlm2error(trans, "14.7");
      }
  if (trans.get_streaming_width() != init->get_streaming_width()) {
    txt << "Streaming width attribute modified during transaction lifetime, "
           "detected in "
        << txt2;
    tlm2error(trans, "14.7");
  }
  if (init->get_gp_option() == tlm::TLM_MIN_PAYLOAD &&
    ...
    txt << "Generic payload option attribute modified during transaction "
           "lifetime, detected in "
        << txt2;
    tlm2error(trans, "14.8 g)");
  }
  if (!m_map[&trans].has_mm) {
    if (trans.has_mm()) {
      txt << "Interconnect component sets a memory manager, but does not clear "
             "it on return, detected in "
          << txt2;
      tlm2error(trans, "14.5 aa)");
    }

    for (unsigned int i = 0; i < tlm::max_num_extensions(); i++)
      // Exclude tlm_endian_context extension from the check because it is not
      // cloned in m_map
      if (i != tlm::tlm_endian_context::ID)
        if (trans.get_extension(i))
          if (!m_map[&trans].gp->get_extension(i)) {
            txt << "Extension set (index = " << i
                << ") without also being deleted in the absence of a memory "
                   "manager, detected in "
                << txt2;
            tlm2error(trans, "14.5 aa)");
          }
  ...
  if (resp_data_ptr)
    for (unsigned int i = 0; i < trans.get_data_length(); i++)
      if (trans.get_data_ptr()[i] != resp_data_ptr[i]) {
        txt << "Transaction data array modified in interconnect component on "
               "response path, detected in "
            << txt2;
        tlm2error(trans, "14.7");
      }
}
...
BOILERPLATE
check_response_path(tlm::tlm_generic_payload &trans, const char *txt2) {
...
BOILERPLATE
get_direct_mem_ptr_pre_checks(tlm::tlm_generic_payload &trans,
                              tlm::tlm_dmi &dmi_data) {
  ...
  if (!dmi_data.is_none_allowed()) {
    txt << "DMI descriptor not properly initialized: granted_access != "
           "DMI_ACCESS_NONE";
  ...
  if (dmi_data.get_read_latency() != sc_core::SC_ZERO_TIME) {
    txt << "DMI descriptor not properly initialized: read_latency != "
           "SC_ZERO_TIME";
    tlm2error(trans, "11.2.5 ac)");
  }
  if (dmi_data.get_write_latency() != sc_core::SC_ZERO_TIME) {
    txt << "DMI descriptor not properly initialized: write_latency != "
           "SC_ZERO_TIME";
    tlm2error(trans, "11.2.5 ac)");
  }
  ...
  if (trans.get_gp_option() == tlm::TLM_FULL_PAYLOAD) {
    /*
    if (trans.is_dmi_allowed())  // Would be rather brutal to flag dmi_allowed
    as an arror for a DMI transaction!
    {
      ...
      tlm2error(trans, "14.8 e) & 14.16");
        ...
        */
    if (trans.get_response_status() != tlm::TLM_INCOMPLETE_RESPONSE) {
      txt << "DMI transaction not properly initialized: response_status != "
             "TLM_INCOMPLETE_RESPONSE";
      tlm2error(trans, "14.8 e) & 14.17 e)");
    }
  } else if (trans.get_gp_option() == tlm::TLM_FULL_PAYLOAD_ACCEPTED) {
    txt << "DMI transaction not properly initialized: gp_option == "
...
get_direct_mem_ptr_post_checks(tlm::tlm_generic_payload &trans,
  ...
  if (init->get_gp_option() == tlm::TLM_MIN_PAYLOAD &&
      trans.get_gp_option() != tlm::TLM_MIN_PAYLOAD) {
    txt << "DMI transaction gp_option attribute value TLM_MIN_PAYLOAD modified "
           "during transaction lifetime";
    tlm2error(trans, "14.8 h)");
  } else if (init->get_gp_option() == tlm::TLM_FULL_PAYLOAD &&
             trans.get_gp_option() == tlm::TLM_MIN_PAYLOAD) {
    txt << "DMI transaction gp_option attribute value changed from "
           "TLM_FULL_PAYLOAD to TLM_MIN_PAYLOAD";
...
transport_dbg_pre_checks(tlm::tlm_generic_payload &trans) {
  ...
  if (trans.get_gp_option() == tlm::TLM_FULL_PAYLOAD) {
    ...
    if (trans.get_response_status() != tlm::TLM_INCOMPLETE_RESPONSE) {
      txt << "Debug transaction not properly initialized: response_status != "
             "TLM_INCOMPLETE_RESPONSE";
      tlm2error(trans, "14.8 f) & 14.17 e)");
    }
  } else if (trans.get_gp_option() == tlm::TLM_FULL_PAYLOAD_ACCEPTED) {
    txt << "Debug transaction not properly initialized: gp_option == "
           "TLM_FULL_PAYLOAD_ACCEPTED";
...
transport_dbg_post_checks(tlm::tlm_generic_payload &trans, unsigned int count) {
  ...
  if (init->get_gp_option() == tlm::TLM_MIN_PAYLOAD &&
      trans.get_gp_option() != tlm::TLM_MIN_PAYLOAD) {
    txt << "Debug transaction gp_option attribute value TLM_MIN_PAYLOAD "
           "modified during transaction lifetime";
    tlm2error(trans, "14.8 h)");
  } else if (init->get_gp_option() == tlm::TLM_FULL_PAYLOAD &&
             trans.get_gp_option() == tlm::TLM_MIN_PAYLOAD) {
    txt << "Debug transaction gp_option attribute value changed from "
           "TLM_FULL_PAYLOAD to TLM_MIN_PAYLOAD";
...
tlm2error(tlm::tlm_generic_payload &trans, const char *ref, bool warning) {
  ...
  txt << "\n  ref_count          = " << dec << trans.get_ref_count()
      << " (int)";
  txt << "\n\n  gp_option          = "
      << (trans.get_gp_option() == tlm::TLM_MIN_PAYLOAD
              ? "TLM_MIN_PAYLOAD"
              : trans.get_gp_option() == tlm::TLM_FULL_PAYLOAD
                    ? "TLM_FULL_PAYLOAD"
                    : "TLM_FULL_PAYLOAD_ACCEPTED");
  txt << "\n  command            = "
