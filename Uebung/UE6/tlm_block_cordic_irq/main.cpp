...
// global emulated interrupt flag
static volatile short int rdy_irq = false;
...
int sc_main(int argc, char *agrv[]) {
  ...
  cpu = Emu_Cpu::get_instance(main_c, irq_forward);
  slave = new Slave_Cordic("target");
  cpu->socket.bind(slave->socket);
  
  /* connect irq to cpu signal slave reading */
  cpu->cordic_irq(irq);
  slave->sig_irq(irq);
